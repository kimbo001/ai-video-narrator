...
const App: React.FC = () => {
  ...
  // Track used media URLs to avoid duplicates
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (let i = 0; i < analyzedScenes.length; i++) {
        ...
        // Continuity fallback: reuse previous scene media
        if (!mediaUrl && i > 0 && scenesWithMedia[i - 1]?.mediaUrl) {
          mediaUrl = scenesWithMedia[scenesWithMedia.length - 1].mediaUrl;
          mediaType = scenesWithMedia[scenesWithMedia.length - 1].mediaType;
        }
        ...
        if (!mediaUrl) {
          ...
          mediaUrl = `https://placehold.co/${width}x${height}/000000/FFFFFF/png?text=No+Media+Found`;
          mediaType = 'image';
        } else {
      ...
      console.error(error);
      setStatus({ step: 'error', message: 'Something went wrong. Check API keys or try again.' });
    }
  };
  ...
  const handleRegenerateScene = async (sceneId: string) => {
    ...
    if (mediaUrl) {
      ...
      alert("No more unique media found for this scene.");
    }
  ...
  const handleFileUpload = (sceneId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setScenes(prev => prev.map(s =>
      s.id === sceneId
        ? { ...s, mediaUrl: objectUrl, mediaType: type, isRegenerating: false }
        : s
    ));
  };
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="e.g. Golden Retriever, Cyberpunk city"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="e.g. text, watermark, people"
                        className="w-full bg-[#0b0e14] border border-zinc-800 rounded-lg py-2 pl-9 pr-3 text-sm text-zinc-200 focus:border-cyan-500 outline-none placeholder:text-zinc-600"
                      />
                    </div>
                  </div>
                        ...
                        <div className={`w-full ${config.orientation === VideoOrientation.Landscape ? 'aspect-video' : 'aspect-[9/16]'} bg-[#0b0e14] rounded-lg overflow-hidden border border-zinc-800 relative`}>
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <button
                              ...
                              className="p-1.5 bg-black/60 rounded-full text-white hover:bg-cyan-500 transition-colors"
                              title="Regenerate Visual"
                            >
                              <RefreshCw className={`w-4 h-4 ${scene.isRegenerating ? 'animate-spin' : ''}`} />
                            </button>
                            <label className="p-1.5 bg-black/60 rounded-full text-white hover:bg-cyan-500 transition-colors cursor-pointer" title="Upload Media">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                            <p className="text-[10px] text-zinc-300 truncate">
                              {config.negativePrompt && <span className="text-red-400 mr-1">!</span>}
                              {scene.visualSearchTerm}
                            </p>
                          </div>
                        </div>
                        <p className="mt-1.5 text-[10px] text-zinc-500 truncate">
                          Scene {idx + 1}: {scene.narration.substring(0, 30)}...
                        </p>
                      </div>
                    ))}
                  </div>
