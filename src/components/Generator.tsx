      ...
      for (let i = 0; i < analyzedScenes.length; i++) {
        ...
        if (!mediaUrl) {
          if (i > 0 && scenesWithMedia[i-1].mediaUrl) {
            ...
            mediaUrl = `https://placeholder.co/${width}x${height}/000000/FFF?text=Scene+1`;
            mediaType = 'image';
          }
        } else {
          if (mediaUrl) {
                  ...
                  <div key={scene.id} className="relative group">
                    <div className={"w-full aspect-video bg-[#0b0e14] rounded-lg overflow-hidden border border-zinc-800 relative " + (scene.isRegenerating ? "opacity-50" : "")}>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-opacity">
                        <button onClick={() => handleRegenerateScene(scene.id)} className="p-1.5 bg-black/60 rounded-full text-white">
